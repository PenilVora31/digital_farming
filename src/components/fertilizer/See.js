...
export default function See({ handleClick, seeDetails, area }) {
  const {
    user: { uid: userId },
  } = useContext(UserContext);
  const { chemical_name, chemical_amount, cost } = seeDetails;
  const history = useHistory();
  const [selectValue, setSelectValue] = useState(null);
  const [inputValue, setInputValue] = useState("0");
  const [priceCost, setPriceCost] = useState(parseInt(cost));
  const [totalCost, setTotalCost] = useState(parseInt(0));
    ...
    <div className="relative bg-white-base  flex md:flex-row flex-col  md:mx-auto mx-4 w-full  justify-center items-center mt-8 shadow-lg rounded-2xl hover:shadow-xl transition duration-500 ease-in-out py-16">
      ...
      <div className="md:w-1/2 md:ml-0 ml-10 md:mt-0 mt-10 w-full  md:m-4">
        <div className="w-full">
          <h1 className="h3 mb-4 text-center">See Land</h1>
          ...
          <div className="flex w-full">
            ...
            <p className="leading-7 mr-4">
              {area * chemical_amount} kg Gypsum{" "}
            </p>
          </div>
        </div>
