...
export default function Nitrogen({ handleClick, nitrogenDetails, area }) {
  const {
    user: { uid: userId },
  } = useContext(UserContext);
  const { chemical_name, chemical_amount, cost } = nitrogenDetails;

  const history = useHistory();
  const [selectValue, setSelectValue] = useState(null);
  const [inputValue, setInputValue] = useState("0");
  const [priceCost, setPriceCost] = useState(cost);
    ...
    <div className="relative bg-white-base  flex md:flex-row flex-col  md:mx-auto mx-4 w-full  justify-center items-center mt-8 shadow-lg rounded-2xl hover:shadow-xl transition duration-500 ease-in-out py-16">
      ...
      <div className="md:w-1/2 md:ml-0 ml-10 md:mt-0 mt-10 w-full  md:m-4">
        <div className="w-full">
          <h1 className="h3 mb-4 text-center">{chemical_name}</h1>
          <div className="flex w-full">
            <p className="li mb-4 flex w-3/5">Requirment per hectare:</p>
            <p className="leading-7">
              {chemical_amount} Nitrogen + {chemical_amount * 2} Phosphorus
            </p>
          </div>
          <div className="flex w-full">
            ...
            <p className="leading-7 ml-4">
              {area * chemical_amount} kg nitrogen +{area * chemical_amount * 2}
              kg phosphorus
            </p>
          </div>
        </div>
        <div className="mt-8 w-full flex flex-col gap-4">
          ...
          >
            <p className="text-center">for salty land</p>
          </div>
          <div
